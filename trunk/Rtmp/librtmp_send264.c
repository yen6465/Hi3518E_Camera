...
int rtmp_write_audio_header(RTMP *rtmp){
    ...
    packet.m_body[2] = 0x11;  //44100Hz 0x1210   48000Hz 单通道 0x1188  
    packet.m_body[3] = 0x88;//0x90;//0x10修改为0x90,2016-1-19
...
double audio_tick_gap = (1024000.0)/(48000.0) + 0.1;
double video_tick_gap = (1000.0 + 1.0)/30.0 + 0.1;
...
int RTMPMulti_Send264(int(*read_h264)(unsigned char *buf, int buf_size),unsigned int tick)
{
	...
	if(get_sps == FALSE)
	{
		...
		video_tick_gap = (1000.0 + 1.0) / (double)metaData.nFrameRate;
